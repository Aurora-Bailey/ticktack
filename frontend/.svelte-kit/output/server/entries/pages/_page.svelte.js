  ...
  $$renderer.component(($$renderer2) => {
    ...
    if (store_get($$store_subs ??= {}, "$rankings", rankings)[selectedTier]?.length) {
      ...
      for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
        ...
        $$renderer2.push(`<tr class="hover:bg-slate-50"><td class="px-4 py-3 font-semibold text-slate-800"><a class="text-blue-600 hover:underline"${attr("href", `/stocks/${row.symbol}`)}>${escape_html(row.symbol)}</a></td><td class="px-4 py-3 text-right font-mono text-slate-700">${escape_html(row.score.toFixed(2))}</td><td class="px-4 py-3 text-right font-mono text-slate-700">$${escape_html(row.price.toFixed(2))}</td><td${attr_class(`px-4 py-3 text-right font-mono ${row.changePct >= 0 ? "text-emerald-600" : "text-rose-600"}`)}>${escape_html(row.changePct.toFixed(2))}%</td></tr>`);
      }
      $$renderer2.push(`<!--]-->`);
    } else {
      $$renderer2.push("<!--[!-->");
    ...
    for (let $$index_2 = 0, $$length = each_array_2.length; $$index_2 < $$length; $$index_2++) {
      ...
      $$renderer2.push(`<li class="flex items-center justify-between rounded-lg bg-slate-100 px-4 py-2 text-sm"><span class="font-semibold text-slate-800">${escape_html(item.symbol)}</span> <span class="font-mono text-slate-600">${escape_html(item.score.toFixed(1))}</span></li>`);
    }
